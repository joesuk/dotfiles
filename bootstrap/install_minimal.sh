...
progsfile="https://raw.githubusercontent.com/joesuk/dotfiles/main/bootstrap/progs_test.csv"
aurhelper="yay"
repobranch="main"
export TERM=ansi
...
usercheck() {
	! { id -u "$name" >/dev/null 2>&1; } ||
		whiptail --title "WARNING" --yes-button "CONTINUE" \
			--no-button "No wait..." \
			--yesno "The user \`$name\` already exists on this system. Can install for a user already existing, but it will OVERWRITE any conflicting settings/dotfiles on the user account.\\n\\nWill NOT overwrite your user files, documents, videos, etc., so don't worry about that, but only click <CONTINUE> if you don't mind your settings being overwritten.\\n\\nNote also that we will change $name's password to the one you just gave." 14 70
}
...
# All this below to get Librewolf installed with add-ons and non-bad settings. (commented out for now)
